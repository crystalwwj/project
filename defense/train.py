...
if __name__ == '__main__':
    ...
    if args.continue_from:  # Starting from previous model
        ...
        optimizer_teacher = torch.optim.SGD(parameters_teacher, lr=args.lr,
        ...
        rnn_type = args.rnn_type.lower()
        assert rnn_type in supported_rnns, "rnn_type should be either lstm, rnn or gru"
        model_teacher = DeepSpeech(rnn_hidden_size=args.hidden_size,
                           nb_layers=args.hidden_layers,
                           labels=labels,
                           rnn_type=supported_rnns[rnn_type],
                           audio_conf=audio_conf,
    ...
    if args.cuda and not args.distributed:
        ...
        model_teacher.cuda()
        model_teacher = torch.nn.parallel.DistributedDataParallel(model_teacher, device_ids=(args.gpu_rank,) if args.rank else None)
        model_student.cuda()
        model_student = torch.nn.parallel.DistributedDataParallel(model_student, device_ids=(args.gpu_rank,) if args.rank else None)

    print(model_student)
    print("Number of parameters: %d" % DeepSpeech.get_param_size(model_student))
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, (data) in enumerate(train_loader, start=start_iter):
            ...
            for stage in range(1):
                model = model_teacher if stage==1 else model_student
                optimizer = optimizer_teacher if stage==1 else optimizer_student

                out = model(inputs)
                out = out.transpose(0, 1)  # TxNxH
                # output to new loader for student 
                # data = [inputs, out, input_percentages, target_sizes]
                # new_loader.append(data)
                ...
                training_temp = 100
                loss = criterion(out/training_temp, targets, sizes, target_sizes)
                loss = loss / inputs.size(0)  # average the loss by minibatch

                # update labels to logits
                targets = out
                ...
                avg_loss += loss_value if stage==2 else 0
            ...
            if args.checkpoint_per_batch > 0 and i > 0 and (i + 1) % args.checkpoint_per_batch == 0 and main_proc:
                ...
                torch.save(DeepSpeech.serialize(model_student, optimizer=optimizer, epoch=epoch, iteration=i,
        ...
        for i, (data) in tqdm(enumerate(test_loader), total=len(test_loader)):
            ...
            out = model_student(inputs)  # NxTxH
        ...
        if args.tensorboard and main_proc:
            ...
            if args.log_params:
                for tag, value in model_student.named_parameters():
                    tag = tag.replace('.', '/')
                    tensorboard_writer.add_histogram(tag, to_np(value), epoch + 1)
                    tensorboard_writer.add_histogram(tag + '/grad', to_np(value.grad), epoch + 1)
        if args.checkpoint and main_proc:
            file_path = '%s/deepspeech_%d.pth.tar' % (save_folder, epoch + 1)
            torch.save(DeepSpeech.serialize(model_student, optimizer=optimizer, epoch=epoch, loss_results=loss_results,
