...
os.makedirs('%saudio/%s' % (opt.output_path, opt.dataset_name), exist_ok=True)
os.makedirs('%ssaved_models/%s' % (opt.output_path, opt.dataset_name), exist_ok=True)
...
if opt.epoch != 0:
    ...
    G.load_state_dict(torch.load('%ssaved_models/%s/G_%d.pth' % (opt.output_path, opt.dataset_name, opt.epoch)))
    D.load_state_dict(torch.load('%ssaved_models/%s/D_%d.pth' % (opt.output_path, opt.dataset_name, opt.epoch)))
else:
    # Initialize weights
...
# lr_scheduler_G = LambdaLR(optimizer_G, lr_lambda=LambdaLR(opt.n_epochs, opt.epoch, opt.decay_epoch).step)
# lr_scheduler_D = LambdaLR(optimizer_D, lr_lambda=LambdaLR(opt.n_epochs, opt.epoch, opt.decay_epoch).step)
...
def sample_audios(batches_done):            # do i need this?
    ...
    output_name = '%saudio/%s/%s.flac' %(opt.output_path, opt.dataset_name, batches_done)
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if opt.checkpoint_interval != -1 and epoch % opt.checkpoint_interval == 0:
        ...
        torch.save(G.state_dict(), '%ssaved_models/%s/G_%d.pth' % (opt.output_path, opt.dataset_name, epoch))
        torch.save(D.state_dict(), '%ssaved_models/%s/D_%d.pth' % (opt.output_path, opt.dataset_name, epoch))
